-- Migration ID: 91_fix_admin_reschedule_reassign_transition
-- Description: Fix public.admin_reschedule_booking_atomic and public.admin_reassign_booking (4-parameter versions) to pass force_override: true in transition_booking metadata to allow admin overrides from accepted/ready statuses. Also fix v_service_id type from UUID to TEXT in admin_reassign_booking.

BEGIN;

-- 1. Redefine public.admin_reschedule_booking_atomic (4-parameter version)
...
DECLARE
    v_tech_id      UUID;
    v_service_id   TEXT; -- Text matching bookings table schema
    v_pool_id      UUID;
    v_max_cap      INTEGER;
...
-- 2. Redefine public.admin_reassign_booking (4-parameter version)
...
DECLARE
    v_service_id   TEXT; -- Fixed: Changed from UUID to TEXT to match schema
    v_pool_id      UUID;
    v_max_cap      INTEGER;
    v_current_load INTEGER;
