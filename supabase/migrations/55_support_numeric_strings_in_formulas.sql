-- Migration ID: 55_support_numeric_strings_in_formulas
-- Description: Update calculate_computed_fields and stage_1_calculate_base_pricing to support
-- numeric strings in formula evaluations (e.g. selected values of dropdown fields).

BEGIN;

-- 1. Re-define public.calculate_computed_fields to handle numeric strings
    ...
    FOR v_field IN SELECT * FROM jsonb_array_elements(p_computed_fields) LOOP
        ...
        IF v_id IS NOT NULL AND v_formula IS NOT NULL AND v_formula != '' THEN
            ...
            FOR v_token IN SELECT DISTINCT (regexp_matches(v_formula, '\{([a-zA-Z0-9_]+)\}', 'g'))[1] LOOP
                DECLARE
                    v_token_text TEXT := v_result ->> v_token;
                BEGIN
                    ...
                    ELSIF v_token_text ~ '^-?[0-9]+(\.[0-9]+)?$' THEN
                        v_replaced_formula := replace(v_replaced_formula, '{' || v_token || '}', v_token_text);
                    ELSE
                        v_replaced_formula := replace(v_replaced_formula, '{' || v_token || '}', '0.0');
                    END IF;
                END;
...
-- 2. Re-define public.stage_1_calculate_base_pricing to handle numeric strings
