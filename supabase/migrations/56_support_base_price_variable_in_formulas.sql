-- Migration ID: 56_support_base_price_variable_in_formulas
-- Description: Redefine stage_1_calculate_base_pricing to support {base_price} token in formulas.
...
BEGIN
    ...
    IF v_formula IS NOT NULL AND length(trim(v_formula)) > 0 THEN
        ...
        FOR v_token IN SELECT DISTINCT (regexp_matches(v_formula, '\{([a-zA-Z0-9_]+)\}', 'g'))[1] LOOP
            ...
            BEGIN
                -- Support base_price variable token by falling back to v_unit_price if not explicitly provided in inputs
